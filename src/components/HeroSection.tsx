...
const CATEGORIES = [
  { name: "Laptops", fontClass: "text-xs xs:text-sm sm:text-base md:text-sm lg:text-xs xl:text-sm 2xl:text-base tracking-wider", spanClass: "" },
  { name: "PC", fontClass: "text-sm xs:text-base sm:text-lg md:text-base lg:text-sm xl:text-base 2xl:text-lg tracking-widest", spanClass: "" },
  { name: "Printers", fontClass: "text-xs xs:text-sm sm:text-base md:text-sm lg:text-xs xl:text-sm 2xl:text-base tracking-wider", spanClass: "" },
  { name: "Parts", fontClass: "text-xs xs:text-sm sm:text-base md:text-sm lg:text-xs xl:text-sm 2xl:text-base tracking-wider", spanClass: "" },
  { name: "Upgrades", fontClass: "text-xs xs:text-sm sm:text-base md:text-sm lg:text-xs xl:text-sm 2xl:text-base tracking-wider", spanClass: "" },
  { name: "Repairs", fontClass: "text-xs xs:text-sm sm:text-base md:text-sm lg:text-xs xl:text-sm 2xl:text-base tracking-wider", spanClass: "" },
  { name: "Accessories", fontClass: "text-[10px] xs:text-xs sm:text-sm md:text-xs lg:text-[10.5px] xl:text-xs 2xl:text-sm tracking-wide", spanClass: "col-span-2 sm:col-span-1 md:col-span-2 lg:col-span-1" },
];
...
function CategoryCard({ cat, index, variants, onSelectService }: CategoryCardProps) {
  const handleClick = () => {
    if (cat.name === "Upgrades") {
      ...
      const pcSection = document.getElementById("pc-section") || document.getElementById("pc-catalog-section");
      if (pcSection) {
        pcSection.scrollIntoView({ behavior: "smooth", block: "start" });
      }
    } else if (cat.name === "Printers") {
      const printerSection = document.getElementById("printers-section") || document.getElementById("printers-catalog-title");
      if (printerSection) {
      ...
      const laptopSection = document.getElementById("products-catalog-section") || document.getElementById("brands-section");
      if (laptopSection) {
  ...
  return (
    ...
    >
      {/* Interactive Subtle Ethereal Glow - Hardware accelerated */}
      ...
      {/* Category Title - Centered & layered safely above effects for 100% clarity */}
      <motion.h2
...
export default function HeroSection() {
  ...
  const [activeServiceModal, setActiveServiceModal] = useState<HeroServiceType>(null);
  ...
  const headingY = useTransform(smoothProgress, [0, 0.5, 1], [0, -35, -80]);
  const headingScale = useTransform(smoothProgress, [0, 0.5, 1], [1, 0.97, 0.92]);
  const headingOpacity = useTransform(smoothProgress, [0, 0.7, 1], [1, 0.88, 0.7]);

  // Scroll animations for Category Boxes
  const categoriesY = useTransform(smoothProgress, [0, 0.5, 1], [0, 18, 45]);
  const categoriesOpacity = useTransform(smoothProgress, [0, 0.8, 1], [1, 0.92, 0.8]);
          ...
          >
            <div className="overflow-hidden">
              <motion.h1
                ...
                className="text-[46px] xs:text-[57.6px] sm:text-[69px] md:text-[92px] lg:text-[123px] xl:text-[134.4px] font-black uppercase text-blue-500 leading-[0.82] tracking-tighter mb-2 fusion-outline"
              >
                FUSION
              </motion.h1>
            </div>
            ...
            <div className="overflow-hidden">
              <motion.h1
                ...
                whileHover={{ scale: 1.01 }}
                className="text-[100px] xs:text-[100px] sm:text-[100px] md:text-[92px] lg:text-[123px] xl:text-[134.4px] font-black uppercase factor-split factor-outline leading-[0.82] tracking-tighter"
              >
                FACTOR
              </motion.h1>
            </div>
